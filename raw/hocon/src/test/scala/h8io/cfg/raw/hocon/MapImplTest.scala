...
class MapImplTest extends AnyFlatSpec with Matchers with Inside with MockFactory {
  "apply" should "return a Node.Scalar object" in {
    ...
    inside(MapImpl(Id.Root, obj, Tag.None(LocationImpl(obj)))("scalar")) {
      case Node.Scalar(Id.Key("scalar", Id.Root), "42", Tag.None(LocationImpl(tagOrigin)), LocationImpl(origin)) =>
        origin should be theSameInstanceAs scalar.origin
        tagOrigin should be theSameInstanceAs scalar.origin
    }
  }
  ...
  it should "return a Node.None object" in {
    ...
    val map = MapImpl(Id.Root, obj, Tag.None(LocationImpl(obj)))
  ...
  it should "return a Node.Seq object" in {
    ...
    inside(MapImpl(Id.Root, obj, Tag.None(LocationImpl(obj)))("seq")) { case seq: Node.ISeq[Id.Key] =>
      seq.tag shouldBe Tag.None(LocationImpl(list))
  ...
  it should "return a Node.Map object" in {
    ...
    inside(MapImpl(Id.Root, cfg, Tag.None(LocationImpl(obj)))("map")) { case map: Node.IMap[Id.Key] =>
      map.iterator.map { node =>
        inside(node) {
          case Node.Scalar(
                Id.Key(key, Id.Key("map", Id.Root)), scalar, Tag.None(LocationImpl(tagOrigin)), LocationImpl(origin)) =>
  ...
  "iterator" should "return a correct sequence of nodes" in {
    ...
    MapImpl(Id.Root, obj, Tag.None(LocationImpl(obj))).iterator.map { node =>
      inside(node) {
        case Node.Scalar(Id.Key(key, Id.Root), scalar, Tag.None(LocationImpl(tagOrigin)), LocationImpl(origin)) =>
          origin should be theSameInstanceAs obj.get(key).origin
          tagOrigin should be theSameInstanceAs obj.get(key).origin
          key -> Some(scalar)
  ...
  "size" should "return the same value as underlying.size" in {
    ...
    MapImpl(Id.Root, obj, Tag.None(LocationImpl(mock[ConfigOrigin]))).size shouldBe 17
  }
  ...
  "location" should "be a wrap on underlying origin object" in {
    ...
    inside(MapImpl(Id.Root, obj, Tag.None(LocationImpl(mock[ConfigOrigin]))).location) { case LocationImpl(origin) =>
      origin should be theSameInstanceAs expectedOrigin
    }
  }
  ...
  "toString" should "delegate call to underlying.render" in {
    ...
    MapImpl(Id.Root, obj, Tag.None(LocationImpl(mock[ConfigOrigin]))).toString should be theSameInstanceAs expected
  }
}
