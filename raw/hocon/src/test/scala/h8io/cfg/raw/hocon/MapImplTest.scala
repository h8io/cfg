...
class MapImplTest extends AnyFlatSpec with Matchers with Inside with MockFactory {
  "apply" should "return a Node.Scalar object" in {
    ...
    inside(MapImpl(Id.Root, obj, None)("scalar")) {
      case Node.Scalar(Id.Key("scalar", Id.Root), "42", None, LocationImpl(scalarOrigin)) =>
  ...
  it should "return a Node.None object" in {
    val map = MapImpl(Id.Root, hocon"""scalar: 13""", None)
  ...
  it should "return a Node.Seq object" in {
    ...
    inside(MapImpl(Id.Root, obj, None)("seq")) { case seq: Node.ISeq[Id.Key] =>
      seq.iterator.zipWithIndex.map { case (value, i) =>
        val expectedOrigin = list.get(i).origin
        val id = seq.id
        inside(value) {
          case Node.Scalar(Id.Index(`i`, `id`), scalar, None, LocationImpl(origin)) =>
  ...
  it should "return a Node.Map object" in {
    ...
    inside(MapImpl(Id.Root, cfg, None)("map")) { case map: Node.IMap[Id.Key] =>
      map.iterator.map { node =>
        inside(node) {
          case Node.Scalar(Id.Key(key, Id.Key("map", Id.Root)), scalar, None, LocationImpl(origin)) =>
  ...
  "iterator" should "return a correct sequence of nodes" in {
    ...
    MapImpl(Id.Root, obj, None).iterator.map { node =>
      inside(node) {
        case Node.Scalar(Id.Key(key, Id.Root), scalar, None, LocationImpl(origin)) =>
          origin should be theSameInstanceAs obj.get(key).origin
          key -> Some(scalar)
  ...
  "size" should "return the same value as underlying.size" in {
    ...
    MapImpl(Id.Root, obj, None).size shouldBe 17
  }
  ...
  "location" should "be a wrap on underlying origin object" in {
    ...
    inside(MapImpl(Id.Root, obj, None).location) { case LocationImpl(origin) =>
      origin should be theSameInstanceAs expectedOrigin
    }
  }
  ...
  "toString" should "delegate call to underlying.render" in {
    ...
    MapImpl(Id.Root, obj, None).toString should be theSameInstanceAs expected
  }
}
