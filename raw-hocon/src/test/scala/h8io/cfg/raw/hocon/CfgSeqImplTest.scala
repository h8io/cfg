...
class CfgSeqImplTest extends AnyFlatSpec with Matchers with Inside with MockFactory {
  "apply" should "return CfgNone if index is negative" in {
    ...
    inside(SeqImpl(Path.Root, list)(-13)) { case Entry.None(Path.Index(-13), OriginImpl(origin)) =>
  ...
  it should "return CfgNone if index is equal to the array size" in {
    ...
    inside(SeqImpl(Path.Root, list)(index)) { case Entry.None(Path.Index(`index`), OriginImpl(origin)) =>
      origin should be theSameInstanceAs list.origin
    }
  }
  ...
  it should "return CfgNull" in {
    ...
    inside(SeqImpl(Path.Root, list)(3)) { case Entry.Null(Path.Index(3), OriginImpl(origin)) =>
  ...
  it should "return CfgScalar" in {
    ...
    inside(SeqImpl(Path.Root, list)(0)) { case Entry.Scalar(Path.Index(0), "three", OriginImpl(origin)) =>
  ...
  it should "return CfgSeq" in {
    ...
    inside(SeqImpl(Path.Root, list)(1)) { case seq: Entry.Seq[?] =>
  ...
  it should "return CfgMap" in {
    ...
    inside(SeqImpl(Path.Root, list)(index)) { case seq: Entry.Map[?] =>
  ...
  "iterator" should "return a correct sequence of entries" in {
    ...
    SeqImpl(Path.Root, list).iterator.zipWithIndex.map { case (entry, i) =>
  ...
  "origin" should "be a wrap on underlying origin object" in {
    ...
    inside(SeqImpl(Path.Root, list).origin) { case OriginImpl(origin) =>
      origin should be theSameInstanceAs expectedOrigin
    }
  }
}
