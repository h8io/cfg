...
trait time {
  ...
  implicit val finiteDurationDecoder: Decoder[FiniteDuration] = durationDecoder.flatMap {
    case fd: FiniteDuration => _ => fd.valid
    case _ => UnexpectedNode[FiniteDuration](_).invalid
  }
