...
class MapDecoderTest extends AnyFlatSpec with Matchers with MockFactory {
  ...
  "MapDecoder" should "return a map of decoded values if all values are decoded successfully" in {
    ...
    (() => map.iterator).expects().returns(
      Iterator("abc" -> "zyx", "def" -> "wvu", "ghi" -> "tsr").map { case (k, v) =>
        Node.Scalar(Id.Key(k, Id.Root), v, tag, MockLocation(s"$v location ($k)"))
  ...
  it should "return a list of all errors if some values are not decoded successfully" in {
    ...
    (() => map.iterator).expects().returns(
      Iterator(
        Node.Scalar(Id.Key("零", Id.Root), "first", tag, MockLocation(s"first location (0)")),
        mapItem,
        Node.Scalar(Id.Key("二", Id.Root), "third", tag, MockLocation(s"third location (2)")),
        seqItem,
        Node.Scalar(Id.Key("四", Id.Root), "fifth", tag, MockLocation(s"fifth location (4)")),
        Node.Scalar(Id.Key("五", Id.Root), "sixth", tag, MockLocation(s"sixth location (5)")),
